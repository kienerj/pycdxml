...
class CdxmlStylerTest(unittest.TestCase):
    ...
    def test_charge_symbol(self):
        ...
        self.assertTrue(filecmp.cmp('files/styler_charge_reference.cdxml', self.charge_out_file, shallow=False),
                        "Generated cdxml file does not match expected outcome.")

    def test_single_node(self):
        styler = cdxml_styler.CDXMLStyler(style_name="ACS 1996")
        styler.apply_style_to_file(self.single_node, outpath=self.single_node_out)
        self.assertTrue(filecmp.cmp('files/single_node_reference.cdxml', self.single_node_out, shallow=False),
                        "Generated cdxml file does not match expected outcome.")

    def setUp(self):
        self.test_file = 'files/styler_test_input.cdxml'
        self.charge_file = 'files/styler_charge.cdxml'
        self.out_file = 'files/styler_test_out.cdxml'
        self.charge_out_file = 'files/styler_charge_out.cdxml'
        self.single_node = 'files/single_node.cdxml'
        self.single_node_out = 'files/single_node_out.cdxml'
